...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        if (result[grade] == null) result += grade to mutableListOf(name)
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((key, value) in mapA) result += key to value

    for ((key, value) in mapB) {
        if (result[key] == null) result += key to value
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val tempMap = mutableMapOf<String, MutableList<Double>>()
    val result = mutableMapOf<String, Double>()

    for ((sale, price) in stockPrices) {
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((name, info) in stuff) {
        if (info.first == kind) {
            if (result.isEmpty()) result = mutableMapOf(kind to (name to info.second))
            else if (result[kind]!!.second > info.second) result = mutableMapOf(kind to (name to info.second))
        }
    }
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    val allInfo = mutableMapOf<String, Int>()

    for (element in list) {
        if (allInfo[element] == null) allInfo += element to 1
