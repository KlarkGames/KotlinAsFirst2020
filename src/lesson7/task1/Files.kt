...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (string in substrings) {
        var counter = 0
        var i = 0
        while (i != (File(inputName).readText().toLowerCase().length) - string.length + 1) {
            var tempSubstring = ""
            for (j in string.indices) {
                tempSubstring += File(inputName).readText().toLowerCase()[i + j]
            }
            if (tempSubstring == string.toLowerCase()) {
                counter++
            }
            i++
        }
        result += string to counter
    }
    return result
}
...
fun centerFile(inputName: String, outputName: String) {
    ...
    println(maxLength)
    for (line in File(inputName).readLines()) {
        for (i in 0 until ((maxLength - line.trim().length) / 2)) {
            writer.write(" ")
        }
